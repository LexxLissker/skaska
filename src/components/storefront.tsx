...
function ImageSlot({ product, className = "" }: { product?: Product; className?: string }) {
  return <div className={"image-slot " + className}>{product?.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <><span className="steam s1" /><span className="steam s2" /><span className="steam s3" /><div className="image-slot__dish">✦</div><div className="image-slot__empty">Фото из Vendure</div></>}</div>;
}
...
export function Storefront({ catalog }: { catalog: StorefrontCatalog }) {
  ...
  return <main className="app-shell"><div className="phone-frame">
    <div className="status-bar"><span>9:30</span><span className="status-icons">◢ ▰</span></div>
    {screen !== "catalog" && <header className="screen-header"><button className="round-button" onClick={back}><Icon name="back" /></button><h1>{screen === "detail" ? selected?.name : screen === "cart" || screen === "checkout" ? "Оформление" : screen === "search" ? "Поиск" : "Меню"}</h1></header>}
    ...
    {screen === "cart" && <div className="scroll-area cart-screen"><section className="order-card"><h2>Состав заказа · {totalQty || 0} {totalQty === 1 ? "товар" : "товаров"}</h2>{cartLoading ? <div className="empty-cart"><p>Загружаем корзину…</p></div> : cart.length === 0 ? <div className="empty-cart"><Icon name="cart" size={32} /><p>В корзине пока пусто</p><button onClick={() => setScreen("catalog")}>Перейти в каталог</button></div> : <>{cart.map((item, index) => <div className="order-line" key={item.lineId || item.id + index}><div><b>{item.name}</b><small>{item.weight === 1000 ? "1 кг" : "0.5 кг"}{item.options.length ? " · " + item.options.join(", ") : ""}</small></div><div className="line-actions"><button onClick={() => void alter(index, 1)}><Icon name="plus" size={13} /></button><strong>{money.format(linePrice(item))}</strong><button className="remove" onClick={() => void alter(index, -1)}><Icon name="minus" size={13} /></button></div></div>)}<div className="total-row"><span>Товары</span><b>{money.format(total)}</b></div><div className="total-row"><span>Доставка</span><span className="muted">После адреса</span></div><div className="total-row grand-total"><b>К оплате</b><b>{money.format(total)}</b></div></>}</section>{cart.length > 0 && <section className="cart-upsell"><h2>С чем подать?</h2><p>Добавьте соус или масло к горячему ужину.</p>{catalog.products.filter((item) => item.categoryId === "lapsha").slice(0, 2).map((item) => <button key={item.id} onClick={() => void add(item)}><span>{item.name}</span><b>+ {money.format(item.price)}</b></button>)}</section>}<div className="sticky-action"><button disabled={!cart.length || cartLoading} onClick={() => setScreen("checkout")}>К оформлению — {money.format(total)}</button></div></div>}
    ...
    {screen !== "detail" && screen !== "checkout" && <nav className="bottom-nav"><button className="brand-button" onClick={() => setScreen("catalog")}><Icon name="logo" /></button><button onClick={() => setScreen("search")} className={screen === "search" ? "is-current" : ""}><Icon name="search" /></button><button onClick={() => setScreen("cart")} className={screen === "cart" ? "is-current" : ""}><Icon name="cart" />{totalQty > 0 && <i>{totalQty}</i>}</button><button aria-label="Написать в поддержку"><Icon name="chat" /></button><button onClick={() => setScreen("menu")} className={screen === "menu" ? "is-current" : ""}><Icon name="menu" /></button></nav>}
  </div></main>;
}
